...
polybar_bg="`echo $eleven_bg | sed 's/#//g'`"
polybar_fg="`echo $eleven_fg | sed 's/#//g'`"
...
apply_rofi() {
	...
	cat > ${PATH_ROFI}/shared/colors.rasi <<- EOF
		* {
		    background:     ${eleven_bg}D9;
		    background-alt: ${eleven_bg}FF;
		    foreground:     ${eleven_fg}D9;
		    border:         ${eleven_bd}FF;
		    selected:       ${accent}FF;
		    active:         ${color2}FF;
		    urgent:         ${color1}FF;
...
apply_dunst() {
	...
	cat >> ${PATH_DUNST}/dunstrc <<- _EOF_
		[urgency_low]
		timeout = 2
		background = "${eleven_bg}CC"
		foreground = "${eleven_fg}D9"
		frame_color = "${eleven_bd}FF"
...
apply_cairodock() {
	...
	sed -i "s/screen border=.*/screen border=1/g" "$HOME"/.config/cairo-dock/current_theme/cairo-dock.conf
	# restart dunst
...
# Notify User -------------------------------
notify_user() {
	dunstify -u normal -h string:x-dunst-stack-tag:applytheme -i /usr/share/packarch/icons/dunst/themes.png "Applying Style : $THEME"
}
