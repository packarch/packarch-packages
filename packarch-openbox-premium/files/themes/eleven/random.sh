...
check_wallpaper() {
	if [[ -d "$WALLDIR" ]]; then
		...
		if [[ "$WFILES" == 0 ]]; then
			dunstify -u normal -h string:x-dunst-stack-tag:noimage -i /usr/share/packarch/icons/dunst/picture.png "There are no wallpapers in : $WALLDIR"
			exit
		fi
	else
		mkdir -p "$WALLDIR"
		dunstify -u normal -h string:x-dunst-stack-tag:noimage -i /usr/share/packarch/icons/dunst/picture.png "Put some wallpapers in : $WALLDIR"
		exit
	fi
}
...
generate_colors() {	
	check_wallpaper
	if [[ `which wal` ]]; then
		dunstify -t 50000 -h string:x-dunst-stack-tag:runpywal -i /usr/share/packarch/icons/dunst/hourglass.png "Generating Colorscheme. Please wait..."
		wal -q -n -s -t -e -i "$WALLDIR"
		if [[ "$?" != 0 ]]; then
			dunstify -u normal -h string:x-dunst-stack-tag:runpywal -i /usr/share/packarch/icons/dunst/palette.png "Failed to generate colorscheme."
			exit
		fi
	else
		dunstify -u normal -h string:x-dunst-stack-tag:runpywal -i /usr/share/packarch/icons/dunst/palette.png "'pywal' is not installed."
		exit
	fi
}
generate_colors
...
altforeground="`pastel color $foreground | pastel darken 0.35 | pastel format hex`"
polybar_bg="`echo $background | sed 's/#//g'`"
polybar_fg="`echo $foreground | sed 's/#//g'`"
accent="$color4"
...
apply_polybar() {
	cat > ${PATH_PBAR}/colors.ini <<- EOF
		...
		BACKGROUND = #CC${polybar_bg}
		FOREGROUND = #FF${polybar_fg}
...
apply_rofi() {
	cat > ${PATH_ROFI}/shared/colors.rasi <<- EOF
		* {
		    ...
		    background-alt: ${altbackground}FF;
		    foreground:     ${foreground}D9;
		    border:         ${background}CC;
		    selected:       ${accent}FF;
		    active:         ${color2}FF;
		    urgent:         ${color1}FF;
		}
...
apply_dunst() {
	...
	cat >> ${PATH_DUNST}/dunstrc <<- _EOF_
		...
		timeout = 0
		background = "${background}D9"
		foreground = "${color1}FF"
		frame_color = "${color1}FF"
	_EOF_
...
apply_openbox() {
	...
	if [[ ! -d "$HOME/.themes/ob-random" ]]; then
		mkdir -p "$HOME/.themes" && cp -r /usr/share/packarch/openbox/ob-random "$HOME/.themes"
	fi
